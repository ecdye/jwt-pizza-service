...
const { middleware: metricsMiddleware, pizzaPurchase } = require('../metrics.js');
orderRouter.use(metricsMiddleware);
...
orderRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    logger.factoryLogger(req.traceId, { reqBody: factoryReqBody, resBody: j });
    if (r.ok) {
      const revenue = orderReq.items.reduce((sum, item) => sum + item.price, 0);
      pizzaPurchase(true, latency, revenue);
      res.send({ order, followLinkToEndChaos: j.reportUrl, jwt: j.jwt });
    } else {
      pizzaPurchase(false, latency, 0);
